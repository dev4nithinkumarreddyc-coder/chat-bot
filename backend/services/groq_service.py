...
class GroqService:
    ...
    def __init__(self):
        ...
        if not self.api_key or self.api_key == "your_groq_api_key_here":
            logger.warning("GROQ_API_KEY is not set or is using the default placeholder value in .env. Responses will fail.")
            self.client = None
        else:
            # Configure the Groq SDK
            self.client = AsyncGroq(api_key=self.api_key)
            self.model_name = "llama-3.1-8b-instant" # Updated to a current supported model
